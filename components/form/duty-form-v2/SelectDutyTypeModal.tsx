...
export const SelectDutyTypeModal: React.FC = memo(() => {
  ...
  const notSelectedDutyTypes = useMemo(() => {
    return dutyTypes.filter((dutyType) =>
      selectedDutyTypes.every((dtTypeId) => dtTypeId !== dutyType.id)
    );
  }, [selectedDutyTypes, dutyTypes]);

  const onSelectDutyType = useCallback(
    (dutyType: TDutyType) => {
      setSelectingDutyType({ id: dutyType.id, name: dutyType.dutyTypeName });
      // close duty modal when open user modal, if not will raise bug in ios
      setOpenSelectDutyTypeModal(false); // dont delete
  ...
  return (
    <MySlideModal onClose={onClose}>
      <Delayed>
        ...
        {!isFetchingDutyTypes && (
          <View>
            {notSelectedDutyTypes.map((dutyType) => (
              <Button
                key={dutyType.id}
                onPress={() => onSelectDutyType(dutyType)}
              >
        ...
        {!isFetchingDutyTypes && dutyTypes.length <= 0 && (
          <NoData message="Không có loại trực" />
        )}
      </Delayed>
    </MySlideModal>
  );
