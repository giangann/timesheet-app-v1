...
export const SelectDutyUsersModal: React.FC = memo(() => {
    ...
    const selectedUsers = useMemo(() => usersWithCheckStatus.filter((user) => user.isChecked), [usersWithCheckStatus])
    ...
    const onSaveBtnPressed = useCallback(() => {
        ...
        const currentDutyType = dutyTypes.filter((field, index) => field.dutyTypeId === selectingDutyTypeId)?.[0]
        ...
        if (!currentDutyType) {
            ...
            // find index
            const fieldIndex = useFieldArrayReturn?.fields?.findIndex((field) => field.dutyTypeId === selectingDutyTypeId) ?? -1
    ...
    const onSelectUser = useCallback((user: TDutyFormAttendanceInfo) => {
        ...
        if (hasUser) selectedUserIdsRef.current = selectedIds.filter((selectedId) => selectedId !== user.id)
                                    ...
                                    <View style={{ gap: 2 }}>
                                      <View style={styles.filterContainer}>
                                        ...
                                        <FilterUserForm
                                          ...
                                          defaultFilterParmas={_defaultFilterParams(selectingDutyType!.id, moment(useFormReturn?.getValues('date')).format("YYYY-MM-DD"))}
                                        />
                                      </View>
                                      {/* Search */}
                                      <TextInput
