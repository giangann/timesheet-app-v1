...
import {
  MonthTimesheet,
  MonthTimesheetList,
  TMonthTimesheetListParams,
} from "@/api/timesheet/type";
import {
  TIMESHEET_FORM_TYPE,
...
export const BasicCalendar = ({
  ...
  const [isFetchingMonthData, setIsFetchingMonthData] =
    useState<boolean>(false);
  const [timesheetMonthData, setTimesheetMonthData] =
    useState<MonthTimesheetList>([]);
  const [selectedDate, setSelectedDate] = useState(INITIAL_DATE);
  const { session, userInfo } = useSession();

  const timesheetDataMap = useMemo(
    () => timsheetDataToMap(timesheetMonthData),
    [timesheetMonthData]
  );
  ...
  const onDayPressHandler = (date: DateData | undefined) => {
    ...
    onFetchForms(
      timesheetDate.leaveFormId,
      timesheetDate.overtimeFormId,
      timesheetDate.dutyFormId
    );
  ...
  return (
    <View>
      {isFetchingMonthData && <ProgressOverlay/>}
      ...
      <Calendar
        ...
        dayComponent={({ date, state }) => {
          ...
          const isFuture =
            yearOfDate === yearToday
              ? monthOfDate === monthToday
                ? dayOfDate > dateToday
                : monthOfDate > monthToday
              : yearOfDate > yearToday;
                ...
                <View style={styles.dots}>
                  {dots.map((dot) => (
                    <View
                      ...
                      style={[
                        styles.dot,
                        { backgroundColor: TIMESHEET_FORM_TYPE_COLOR[dot] },
                      ]}
                    />
                  ))}
                </View>
