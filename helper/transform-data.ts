...
import {
  TDutyForm,
  TDutyFormDetailDutyType,
  TLeaveForm,
} from "@/api/form/types";
import { arrayStringToString, combineDateAndTimeToDateObject } from "./common";

export function weekCalendarToEventItems(
  weekCalendars: TWeekCalendar[]
): EventItem[] {
  const res: EventItem[] = [];
...
export function dutyFormToEventItems(dutyForms: TDutyForm[]): EventItem[] {
  ...
  dutyForms.forEach((form) => {
    const eventItem: EventItem = {
      ...
      title: `${
        form.salaryCoefficientTypeName
      } (x${form.salaryCoefficient.toFixed(2)}); ${dutyTypeToEventTitle(
        form.dutyTypes
      )};`,
      color: EVENT_COLOR[EVENT_ITEM_PREFIX.DUTY_FORM],
      start: {
        dateTime: combineDateAndTimeToDateObject(
          form.date,
          form.startTime
        ).toISOString(),
      },
...
export function leaveFormToEventItems(leaveForm: TLeaveForm[]): EventItem[] {
  ...
  leaveForm.forEach((form) => {
    const eventItem: EventItem = {
      ...
      title: `${form.leaveFormTypeName} - ${form.userName}, P.${
        form.userTeam.name
      } ${form.note ?? `- ${form.note}`}`,
...
export function weekCalendarUsersToUserFields(
  weekCalendarUsers: TWeekCalendarDetail["users"]
): TWeekCalendarCreateFormFieldsUser[] {
...
export function dutyTypesToUsers(
  dutyTypes: TDutyFormDetailDutyType[]
): TDutyFormDetailDutyType["users"] {
...
export function dutyTypeUsersToName(
  dtTypeUsers: TDutyFormDetailDutyType["users"]
): string {
  return dtTypeUsers.map((user) => user.name).join(", ");
}

export function dutyTypeToEventTitle(
  dutyTypes: TDutyFormDetailDutyType[]
): string {
  let res = "";

  dutyTypes.forEach((dutyType) => {
    res += `• ${dutyType.dutyTypeName} (${dutyTypeUsersToName(
      dutyType.users
    )}) `;
  });
  return res;
